...
local hasFreecam = GetResourceState('fivem-freecam') == 'started'

local function setFreecamActive(active)
    if hasFreecam then
        pcall(function()
            exports['fivem-freecam']:SetActive(active)
        end)
    end
end

local function getCamRotationZ()
    if hasFreecam then
        local ok, rot = pcall(function()
            return exports['fivem-freecam']:GetRotation(2).z
        end)
...
---@param data? {onCreated?:function, onCanceled?:function}
local function closeCreator(cancel, data)
	...
	elseif data and data.onCanceled then
		data.onCanceled()
	end
    ...
    while creatorActive do
        ...
        if displayMode == 3 or displayMode == 4 then
            if alignMovementWithCamera then
                local rightX, rightY = getRelativePos(vec2(xCoord, yCoord), vec2(xCoord + 2, yCoord), getCamRotationZ())
                local forwardX, forwardY = getRelativePos(vec2(xCoord, yCoord), vec2(xCoord, yCoord + 2), getCamRotationZ())
        ...
        if controlsActive then
            ...
            elseif IsDisabledControlPressed(0, 188) then --- arrow up
                ...
                if alignMovementWithCamera then
                    local newX, newY = getRelativePos(vec2(xCoord, yCoord), vec2(xCoord, yCoord + lStep), getCamRotationZ())
            ...
            elseif IsDisabledControlPressed(0, 190) then --- arrow right
                ...
                if alignMovementWithCamera then
                    local newX, newY = getRelativePos(vec2(xCoord, yCoord), vec2(xCoord + lStep, yCoord), getCamRotationZ())
