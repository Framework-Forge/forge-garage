...
---@param ignoreZone? boolean
---@return promise?
function spawnPoint.create(zone, required, existingPoint, vehicleTypes, ignoreZone)
    if not vehicleTypes or type(vehicleTypes) ~= 'table' then return end
    if not ignoreZone and not zone then return end
    ...
    CreateThread(function()
        ...
        while busy do
            ...
            local inZone = true
            if not ignoreZone then
                inZone = glm_polygon_contains(polygon, CurrentCoords, zone.thickness / 4)
            end
