RegisterNetEvent('forge_garage:server:buyVehicle', function(vehData)
    ...
    local license = Player and ((Player.PlayerData and Player.PlayerData.license) or Player.license) or ""
    ...
    if pr_lib.framework.RemovePlayerAccountBalance(source, 'bank', vehData.price, "Job vehicle purchase") then
        ...
        utils.notify(source, GarageBridge.locale('notify.success.vehicleshop.buyVehicle', vehData.label, vehData.price), 'success', 8000)
    end
end)
